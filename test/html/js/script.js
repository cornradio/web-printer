...
function setupEventListeners() {
    ...
    document.getElementById('refreshPrintersBtn').onclick = () => {
        showStatus('正在刷新打印机列表...');
        refreshPrinters();
    };
...
function exportToPNG() {
    ...
    // Check if transparent export is requested
    const exportTransparent = document.getElementById('transparentExport').checked;
    if (exportTransparent) {
        const imageData = ctx.getImageData(0, 0, exportCanvas.width, exportCanvas.height);
        const data = imageData.data;
        const threshold = 250; // White threshold
